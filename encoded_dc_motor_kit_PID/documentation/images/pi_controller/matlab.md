...
intergrator_num = [205.25, 13.58];
intergrator_den = [1 0];
integrator = tf( intergrator_num, intergrator_den);

PI_gs = integrator;

PI_gs = parallel( integrator, k)

gs_top = series( PI_gs, gs)
...
% Discretize gs_lc
gs_lc_discrete = c2d(PI_gs, 0.01, 'tustin')
