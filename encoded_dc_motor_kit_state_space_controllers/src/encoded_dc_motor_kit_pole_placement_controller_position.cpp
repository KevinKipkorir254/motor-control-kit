...
private:
    ...
    void update_shaft_state_and_control_value(const sensor_msgs::msg::JointState &msg)
    {
        ...
        // get control value
        double control_output = update_control_value( shaft_position_, shaft_position_);
    ...
    double update_control_value( double shaft_position, double shaft_velocity)
    {

        double error = reference_velocity - shaft_position; // error = r - y
        ...
        state_space[0] = shaft_position;  //position
        state_space[1] = shaft_velocity;  //velocity
        state_space[2] = (shaft_velocity - yn_1)/period.seconds();  //acceleration
        state_space_total = ((state_space[0] * gains_[0]) + (state_space[1] * gains_[1]));// + (state_space[2] * gains_[2]));

        error_integral += error * period.seconds();
        double Khat_result = error_integral * gains_[3];
        ...
        state_space_total_message.data.push_back(state_space[2]);
    ...
    // DATA STORAGE
    double state_space[3] = {0.0, 0.0, 0.0};
    double state_space_total = 0.0;
    double gains_[4] = { -16.9536, -16.4371, -12.1457, 27.9470};
